...
class DTN:
	def __init__(self):
		...
		self.discriminator.compile(loss='crossentropy', optimizer=optimizer, metrics=['accuracy'])

		self.encoder_f = self.build_encoder_f()
		...
		self.dtn.compile(loss='binary_crossentropy', optimizer=optimizer)
		...
		def build_encoder_f(self):
			...
			model.add(Conv2D(16, (4,4), strides=(2,2), padding='same', kernel_initializer=init))
			model.add(LeakyReLU(alpha=0.2))

			model.add(Conv2D(8, (4,4), strides=(2,2), padding='same', kernel_initializer=init))
			model.add(LeakyReLU(alpha=0.2))
