'''
Source: https://github.com/davrempe/domain-transfer-net
'''
...
def create_emoji(i):
    ...
    try:
        urllib.request.urlretrieve(request, './emoji_data/emoji_{}.png'.format(i[0]+187560))
        pbar.update(1)
...
if __name__ == '__main__':
    ...
    st = time.time()
    pool.map(create_emoji, list(zip(np.arange(num_emojis), random.randint(0,500,num_emojis))))
    pool.close()
    pool.join()
    pbar.close()
